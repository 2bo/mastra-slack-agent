...
sequenceDiagram
  ...
  participant MRS as mention-response-service.ts
  participant PR as agent-result-presenter.ts
  ...
  MH->>MRS: runMentionResponseFlow(...)
  MRS->>MRS: generateThreadId(channel, thread_ts, ts)
  MRS->>S: 「Processing...」親メッセージ投稿
  MRS->>CS: streamToSlack({ ... })
  CS->>S: startStream
  ...
  MRS->>PR: presentMentionResult(result)
  PR->>S: Processingメッセージ削除
```

### 解説

1. `handleMention` が Slack イベントを受け取り、入力テキストを整形します。
2. `runMentionResponseFlow` がメモリスコープ用 `threadId` (`channel:thread_ts(or ts)`) を生成します。
3. `streamToSlack` が `chat.startStream` / `chat.appendStream` / `chat.stopStream` を管理します。
...
6. 最後に `presentMentionResult` / `presentActionResult` が経路別の Slack UI 反映を担当します。
7. ツール不要なら最終テキスト表示後、`Processing...` が削除されて終了します。
...
flowchart TD
  ...
  D --> E[mention-response-service が承認UI投稿<br/>postApprovalRequest]
  ...
  L --> M[declineToolCall + stream result]
  M --> N[拒否後の最終応答を表示]
```
...
5. Reject の場合は `declineToolCall(runId, toolCallId)` を実行し、拒否後の応答を表示します。

## 4. レイヤー責務（実装上の分離）

```mermaid
flowchart LR
  SL[Slack Layer<br/>src/slack/handlers + ui] --> SV[Service Layer<br/>src/slack/services + src/mastra/services]
...
1. Slack Layer はイベント受信・UI更新（投稿/更新）を担当します。
2. Service Layer はメンション応答のオーケストレーション、ストリーム解釈、`tool-call-approval` 検知を担当します。
3. Mastra Layer は LLM 推論・メモリ・ツール実行（Calendar API）を担当します。

## 5. 主要ファイル対応表

- `src/index.ts`: 初期化とハンドラー登録
- `src/slack/handlers/mention-handler.ts`: メンション受信と入力検証
- `src/slack/services/mention-response-service.ts`: メンション応答フローのオーケストレーション
- `src/slack/services/agent-result-presenter.ts`: 実行結果ごとの Slack UI 反映
